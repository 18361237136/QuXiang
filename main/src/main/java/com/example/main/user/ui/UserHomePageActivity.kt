...
import com.example.main.report.ReportActivity
...
import kotlinx.android.synthetic.main.loading_footer.*
...
class UserHomePageActivity : BaseActivity(), LoadDataListener {
    ...
    private var titleOffsetChangeListener: AppBarLayout.OnOffsetChangedListener = object : AppBarLayout.OnOffsetChangedListener {
        ...
        override fun onOffsetChanged(appBarLayout: AppBarLayout, verticalOffset: Int) {
            ...
            if(collapsingToolbar.height+verticalOffset<collapsingToolbar.scrimVisibleHeightTrigger){
                ...
                if(!isUserBgImageDark&&isToolbarAndStatusbarIconDark){// 用户信息和状态栏合并
                    setToolbarAndStatusbarIntoLight()
    ...
    private fun setToolbarAndStatusbarIntoLight() {
    ...
    /**
     * 当关注状态发生改变的时候，刷新当前界面用户的粉丝数量
     */
    private fun setFollowersCountChange(changeCount: Int) {
        try {
            val followerInfo = userFeedsText.text.toString()
